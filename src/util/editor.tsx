...
export function initializeEditor(monaco: Monaco) {
	...
	monaco.editor.defineTheme("grau", {
		base: "vs",
		inherit: true,
		rules: [
	...
	monaco.editor.defineTheme("grau-dark", {
		base: "vs-dark",
		inherit: true,
		rules: [
			{ token: "keyword", foreground: "#e600a4" },
	...
	monaco.languages.registerCompletionItemProvider("surrealql", {
		...
		provideCompletionItems: async (model, position, context) => {
			...
			const linePrefix = model
				...
				.slice(0, Math.max(0, position.column));
			const isAuto =
				context.triggerKind ===
			...
			if (
				isAuto &&
				!tablePrefixes.some((pre) => linePrefix.toUpperCase().endsWith(pre))
			) {
				return;
			}
	...
	monaco.languages.registerCompletionItemProvider("surrealql", {
		...
		provideCompletionItems(_, position, context) {
			...
			const isAuto =
				context.triggerKind ===
				languages.CompletionTriggerKind.TriggerCharacter;
			const suggestions: languages.CompletionItem[] = variableNames.map(
				(variableName) => ({
					label: `$${variableName}`,
 ...
 */
export function configureQueryEditor(
