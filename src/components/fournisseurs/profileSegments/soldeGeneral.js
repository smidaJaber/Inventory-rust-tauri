...
class profileGeneral extends Component {
	...
	//fn listefournissuers rapport
	/*getAllFournisseurs() {
    fetch("/api/mariem/fournisseurs")
      .then((res) => res.json())
      .then((fourns) => this.setState({ listeFournis: fourns }));
  }
  //fn caisse
  getCaisse() {
    ...
    fetch("/api/mariem/caisses")
      .then((res) => res.json())
      .then((caisses) => this.setState({ listeCaisse: caisses, loadingCaisse: false }));
  }
  getLieuStockage() {
  ...
  getCategorie() {
    ...
    fetch("/api/mariem/produits")
      .then((res) => res.json())
      .then((prods) => this.setState({ listeProduit: prods, loadingProduit: false }));
  }*/
	...
	//fn payement
	/*getModePayement() {
    this.setState({ loadingModePayement: true });
    fetch("/api/mariem/paiements")
      .then((res) => res.json())
      .then((pays) => this.setState({ listeModePayement: pays, loadingModePayement: false }));
  }*/
    ...
    fetch("/api/mariem/emballages")
      .then((res) => res.json())
      .then((embs) => this.setState({ listeEmballage: embs, loadingEmballage: false }));
  }*/
	...
	getGlobalTotaux(f) {
		this.setState({ loadingTotaux: true }, () => {
			if (
				this.props.globalDataProfile.AllRessAchat &&
				this.props.globalDataProfile.AllOppsAchat
			) {
				const AllOpps =
					f === "tous" || parseInt(f) === 0
						? this.props.globalDataProfile.AllOppsAchat
						: this.props.globalDataProfile.AllOppsAchat.filter((OA) => {
								return parseInt(f) === parseInt(OA.IDFournisseur);
						  });
				const AllRess =
					f === "tous" || parseInt(f) === 0
						? this.props.globalDataProfile.AllRessAchat
						: this.props.globalDataProfile.AllRessAchat.filter((OR) => {
								return parseInt(f) === parseInt(OR.IDFornisseur);
						  });
				...
				AllOpps.map((OA) => {
					oppmont += OA.montant;
					oppcaisse += OA.nbrCaisse;
					oppemb += OA.nbrEmballage;
					oppmat1 += OA.nbrM1;
					oppmat2 += OA.nbrM2;
				});
				AllRess.map((OR) => {
					oppRessmont += OR.montant;
					oppRessmat += OR.nbrMatriel;
					oppRessemb += OR.nbrEmb;
					oppResscaisse += OR.nbrCS + OR.nbrCN + OR.nbrCA;
				});

				/*
		const oppmont =
			this.props.globalDataProfile.AllOppsAchat &&
			this.props.globalDataProfile.AllOppsAchat.reduce((tot, opp) => {
				return parseInt(f) === 0
					? tot + opp.montant
					: f === "tous"
					? tot + opp.montant
					: parseInt(f) === parseInt(opp.IDFournisseur)
					? tot + opp.montant
					: tot + 0;
			}, 0);
		const oppcaisse =
		...
		const oppRessmont =
			this.props.globalDataProfile.AllRessAchat &&
			this.props.globalDataProfile.AllRessAchat.reduce((tot, opp) => {
				return parseInt(f) === 0
					? tot + opp.montant
					: f === "tous"
					? tot + opp.montant
					: parseInt(f) === parseInt(opp.IDFornisseur)
					? tot + opp.montant
					: tot + 0;
			}, 0);
		const oppRessmat =
		...
		const oppResscaisse =
			...
			this.props.globalDataProfile.AllRessAchat.reduce((tot, opp) => {
				return parseInt(f) === 0
					? tot + opp.nbrCS + opp.nbrCN + opp.nbrCA
					: f === "tous"
					? tot + opp.nbrCS + opp.nbrCN + opp.nbrCA
					: parseInt(f) === parseInt(opp.IDFornisseur)
					? tot + opp.nbrCS + opp.nbrCN + opp.nbrCA
...
*/
				this.setState({
					entreeCaisse: oppcaisse || 0,
					sortieCaisse: oppResscaisse || 0,
					//produit
					entreeProduit: oppmont || 0,
					sortieProduit: oppRessmont || 0,

					//emballage
					entreeEmballage: oppemb || 0,
					sortieEmballage: oppRessemb || 0,
					//materiel
					entreeMateriel: (oppmat1 || 0) + (oppmat2 || 0),
					sortieMateriel: oppRessmat || 0,
	...
	getTotaux() {
		this.setState({ loadingTotaux: true }, () => {
			fetch("/api/mariem/soldetotalfournisseur/" + this.props.idFournisseur)
				.then((res) => res.json())
				.then((tots) => {
					this.setState({
						entreeCaisse: tots[1][0].oppcaisse || 0,
						sortieCaisse: tots[6][0].resscaisse || 0,
						//produit
						entreeProduit: tots[0][0].oppmont || 0,
						sortieProduit: tots[5][0].ressmont || 0,
						//payement
						// entreePaiement: tots.,
						//sortiePaiement: tots.,
						//soldePaiement: tots.,
						//emballage
						entreeEmballage: tots[2][0].oppemb || 0,
						sortieEmballage: tots[7][0].ressemb || 0,
						//materiel
						entreeMateriel:
							(tots[3][0].oppmat1 || 0) + (tots[4][0].oppmat2 || 0),
						sortieMateriel: tots[8][0].ressmat || 0,
			...
			return (
				<div>
										...
										<Grid item xs={12}>
											...
											<BottomNavigation showLabels>
												...
												<BottomNavigationAction
													...
													label={
														<strong style={{ fontSize: "0.99rem" }}>
															{(this.state.loadingTotaux && "Calcule..") ||
					...
					>
						...
						<div id="RapportDivToPrint">
							{this.props.nameFor && this.props.nameFor !== "" && (
								<div id="RapportOne">
									{" "}
									<span
										style={{
									...
									<br />
									<center>
										<h3>Rapport {this.props.nameFor}</h3>
									</center>
									<Row
										style={{
											display: "grid",
												...
												if (rpO.titre === "Montant") {
													return (
														...
														>
															...
															>
																<FaTrash color="red" className="BTNTODELETE" />
															</span>
												...
												if (rpO.titre === "Materiel") {
													return (
														...
														>
															...
															>
																Materiel :{" "}
																{rpO.mat !== "" ? rpO.mat : " Tous materiaux "}{" "}
															</span>
															<span
																className="delete-line-rapport"
																onClick={(e) => {
												...
												{this.state.rapportTot.length > 0 &&
													this.state.listeNewFournis.map((frns, i) => (
														<tr>
															...
															>
																{this.state.listeFournis.map(
																	({ IDFournisseurProd, nom }) => {
																		if (IDFournisseurProd === frns.idf) {
																			return nom;
																		}
																	}
																)}
					...
					<Modal isOpen={this.state.preparingRapport}>
						<ModalHeader>Rappor</ModalHeader>
						<ModalBody>
