...
const getLeft = (bounds?: SplitBounds) =>
	typeof bounds === "number" ? bounds : bounds?.[0];
const getRight = (bounds?: SplitBounds) =>
	typeof bounds === "number" ? bounds : bounds?.[1];
//suggs by GPT
/////////////////////////TODO : enhance and remove the buff size
export function Splitter(props: SplitterProps) {
	...
	const recomputeSizes = useStable((values: SplitValues) => {
		...
		const clampLeft = (value: number) =>
			clamp(
				value,
				getLeft(props.minSize) || 0,
				getLeft(props.maxSize) || totalSize - rightReserve - buffer
			);
		const clampRight = (value: number) =>
			clamp(
				value,
				getRight(props.minSize) || 0,
				getRight(props.maxSize) || totalSize - leftReserve - buffer
			);
