...
import { GrauEnvironment } from "~/typings";
...
export function Environments({ opened, onClose, onSave }: EnvironmentsProps) {
	...
	const [environments, setEnvironments] = useImmer<GrauEnvironment[]>([]);
		...
		>
			<Grid h={350} style={{ gap: 12 }}>
				<Grid.Col
					...
					span={4}
					sx={(theme) => ({
						borderRight: `2px solid ${theme.fn.themeColor(
							isLight ? "light.0" : "dark.4"
						)}`,
						position: "relative",
					})}
				>
