...
function getTabIcon(tab: GrauTab) {
...
export function Selector({
	...
	const environment =
		tab && environments.find((env) => env.id === tab.environment);
	...
	const filteredTabs = useMemo(() => {
		...
		return tabs.filter(
			(tab) =>
				tab.environment === viewingEnv &&
				(!needle || tab.name.toLowerCase().includes(needle))
		);
	...
	const saveTabOrder = useStable((order: GrauTab[]) => {
				...
				<Popover.Dropdown px="xs">
					<SimpleGrid cols={2}>
									...
									{environments.map((item) => {
										...
										return (
											<Button
												...
												color={
													isActive ? (isLight ? "light.1" : "dark.7") : "light"
												}
						...
						<Box mih={235} mah={300}>
							...
							<ScrollArea
								h={tabSearch ? "calc(100% - 102px)" : "calc(100% - 54px)"}
							>
								<Stack spacing={6}>
										...
										{({ item, handleProps }) => {
											...
											return (
												<Button
													...
													c={isLight ? "black" : "white"}
													color={
														isRenaming ? "light" : isActive ? "pink" : "light"
													}
													variant={
														isRenaming
															? "outline"
															...
															>
																...
																<Menu.Dropdown onMouseDown={stopPropagation}>
																	...
																	<Menu.Item
																		icon={
																			<Icon
																				path={item.pinned ? mdiPinOff : mdiPin}
																			/>
																		}
