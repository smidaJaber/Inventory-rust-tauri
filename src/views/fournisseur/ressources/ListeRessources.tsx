...
export interface ListeRessourcesProps {
	refreshId: number;
...
export default function ListeRessources(props: ListeRessourcesProps) {
	...
	let data = useProfileStoreValue((state) => state.listRessourceAchat);
	...
	const fetchRecords = useStable(async () => {
		...
		if (startAt > 0) {
			data = useAsyncMemo(async () => {
				return await getAllTableressourceachats(startAt, null);
			}, []);

			fetchQuery += ` START ${startAt}`;
		}
		console.log("runing req..");
...
function useAsyncMemo(
	arg0: () => Promise<import("~/bindings").Tableressourceachat[]>,
	arg1: never[]
): import("~/bindings").Tableressourceachat[] {
